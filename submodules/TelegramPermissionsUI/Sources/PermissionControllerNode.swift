            ...
            switch state {
                case let .permission(permission):
                    if permission?.kind.rawValue != self.contentNode?.kind {
                        if let dataState = permission {
                            ...
                            let contentNode = PermissionContentNode(context: self.context, theme: self.presentationData.theme, strings: self.presentationData.strings, kind: dataState.kind.rawValue, icon: .image(icon), title: title, text: text, buttonTitle: buttonTitle, secondaryButtonTitle: nil, buttonAction: { [weak self = self] in
                                self?.allow?()
                ...
                case let .custom(icon, title, subtitle, text, buttonTitle, secondaryButtonTitle, footerText):
                    if let contentNode = self.contentNode {
                        ...
                        let contentNode = PermissionContentNode(context: self.context, theme: self.presentationData.theme, strings: self.presentationData.strings, kind: 0, icon: icon, title: title, subtitle: subtitle, text: text, buttonTitle: buttonTitle, secondaryButtonTitle: secondaryButtonTitle, footerText: footerText, buttonAction: { [weak self = self] in
                            self?.allow?()
                        }, openPrivacyPolicy: secondaryButtonTitle != nil ? { [weak self = self] in
                            self?.dismiss?()
                        } : nil)
                        self.insertSubnode(contentNode, at: 0)
                        contentNode.updateLayout(size: contentFrame.size, insets: insets, transition: .immediate)
